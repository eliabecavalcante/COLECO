...
// Fallback caso a API ainda não esteja rodando 100% ou a internet caia na rua
const fallbackColetas = [
  { id: '#ELO-12345', doador: 'Empresa Alfa', local: 'Messejana', material: 'CPUs e cabos', status: 'Agendada' },
  { id: '#ELO-12346', doador: 'Faculdade Beta', local: 'Aldeota', material: 'Monitores', status: 'Em Rota' },
  { id: '#ELO-12347', doador: 'Escola Gama', local: 'Centro', material: 'Ponto cheio', status: 'Pendente' },
];
...
export default function ColetorMobileApp() {
  ...
  const handleLogout = () => {
    // Redireciona o usuário para a tela de login segura
    router.push('/admin');
  };
  ...
  const getStatusBadge = (status: string) => {
    switch (status?.toLowerCase()) {
      ...
      case 'em rota':
      case 'em análise':
        return <span className="bg-amber-100 text-amber-700 font-bold px-3 py-1 rounded-md text-[10px] uppercase tracking-wider border border-amber-200 animate-pulse">{status}</span>;
      case 'coletada':
      case 'concluída':
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] sm:bg-[#E2E8F0] font-sans text-slate-800 flex justify-center sm:items-center sm:py-8">
      
      {/* TELA INTEIRA NO MOBILE / MOCKUP DO CELULAR NO DESKTOP */}
      <div className="w-full h-[100dvh] sm:h-[850px] sm:max-w-[400px] bg-[#F8FAFC] sm:rounded-[40px] sm:shadow-2xl relative overflow-hidden flex flex-col sm:border-[8px] border-slate-800">
        ...
        <main className="flex-1 overflow-y-auto pb-28 bg-slate-50">
          ...
          {activeTab === 'rota' ? (
                    ...
                    >
                      <div className="flex justify-between items-start mb-4">
                        <div>
                          ...
                          <p className="text-slate-500 text-sm font-medium">{coleta.local || coleta.bairro}</p>
                        </div>
                        <div>
                      ...
                      <div className="bg-slate-50 rounded-lg p-3 mb-5 border border-slate-100">
                        ...
                        <p className="text-slate-700 text-sm font-semibold">
                          {coleta.material || 'Material eletrônico diverso'}
                        </p>
                      </div>
            ...
            <div className="p-6 h-full flex flex-col">
              <div className="bg-white rounded-3xl shadow-sm border border-slate-100 p-8 flex flex-col items-center text-center mt-4">
                ...
                <span className="bg-slate-100 text-slate-600 border border-slate-200 text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-wider mt-2 mb-6">
                  Coletor de Frota
                </span>

                <div className="w-full space-y-3 text-left">
                  <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
                    <span className="block text-xs font-bold text-slate-400 uppercase tracking-wider">Veículo Padrão</span>
                    <span className="block text-sm font-semibold text-slate-700 mt-1">Fiorino Branca (ABC-1234)</span>
                  </div>
                  <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
                    <span className="block text-xs font-bold text-slate-400 uppercase tracking-wider">Região de Atuação</span>
                    <span className="block text-sm font-semibold text-slate-700 mt-1">Fortaleza e Região Metropolitana</span>
                  </div>
                </div>
        ...
        {/* BOTTOM NAVIGATION BAR (Fixo no rodapé nativo) */}
        <nav 
          className="absolute bottom-0 left-0 right-0 bg-white/90 backdrop-blur-xl border-t border-slate-200 px-6 pt-4 flex justify-around items-center z-20 shadow-[0_-10px_40px_rgba(0,0,0,0.05)]"
          style={{ paddingBottom: 'max(1.2rem, env(safe-area-inset-bottom))' }}
        >
          <button 
            onClick={() => setActiveTab('rota')}
            className={`flex flex-col items-center gap-1 transition-all ${activeTab === 'rota' ? 'text-blue-600 scale-110' : 'text-slate-400 hover:text-slate-600'}`}
          >
          ...
          <button 
            ...
            className={`flex flex-col items-center gap-1 transition-all ${activeTab === 'perfil' ? 'text-blue-600 scale-110' : 'text-slate-400 hover:text-slate-600'}`}
          >
